...
- **高性能渲染**：基于 Tiptap / ProseMirror 内核（离线 bundle，黑盒适配器保持原编辑器 API 不变），支持完整的 Markdown 语法与代码语法高亮（highlight.js 常用语言内置）；**任何一行的行首**——包括用 Enter 换出来的软换行行——输入 `# `、`- `、`1. `、`> ` 都会当场渲染成标题 / 列表 / 引用，与刷新后的结果一致。
...
- **悬浮助手组**：
  ...
  - **可配置隐藏**：`DUMBPAD_HIDDEN_FLOATING_ACTIONS` 用按钮 id 黑名单收起不需要的功能入口（目录 / 复制 / 今日草稿 / 快速记录 / Thoughts / 反思），只切 `hidden`、不删代码，改回配置即恢复。默认收起「快速记录（剪贴板）」；「反思」默认可见但仍是占位（点击提示开发中）。
...
- 在软换行出来的那一行**行首**输入 `# ` / `- ` / `1. ` / `> `：当场变成标题 / 列表 / 引用，且刷新后一模一样；标记打在行中间（如 `正文# `）不应触发；`---` 分隔线与 ``` 围栏仍按整块语法处理（不在实时拆块范围内）。
