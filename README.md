...
- **AI 元数据**：创建 Thought 后异步生成摘要、实体、主题、意图、关键词、标签和 embedding；更新 Thought 后可在 AI 面板手动重新运行。
...
- **PWA 支持**：可作为应用安装到手机或桌面，支持离线查看及沉浸式全屏体验。Service Worker 会缓存核心静态资源，并预热较大的稳定资源（中文字体、代码高亮主包），减少手机 PWA 后续打开时的重复下载。
