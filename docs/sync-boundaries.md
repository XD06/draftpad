...
- 删除 notepad：删除元数据和正文文件，广播 `notepad_change`。
...
- 删除 Thought：删除本体、AI meta、本 thought 的 relation、suppressed relation，并清理其他 thought 指向它的 relation/suppressed 引用，广播 `thoughts_update:delete`。
...
- 修改 Thought 后自动 `queueThought(id, "update")`。
