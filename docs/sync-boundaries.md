...
- 创建 notepad：前端可先生成安全 id 并本地乐观插入；后端写 `notepads.json` 和初始内容文件后返回最终 notepad。
...
- `notes_update`：notepad 正文发生变化，其他客户端刷新或更新内容。保存产生的事件应携带 `version`，并尽量携带 `saveId/contentHash` 用于客户端识别自己的保存确认。
...
- 前端只应在本地未保存内容和远端保存内容确实不一致时提示冲突。自己的保存确认、无版本的旧式实时 WS 消息、同内容远端版本都不应触发冲突 toast。
- Notepad 保存遇到 `409` 时，前端应把服务端 `currentVersion` 记录到本地缓存的 `remoteVersion`，并标记 `conflict=true`。
- 加载本地 dirty 缓存时，如果远端版本更高但正文一致，应清理 dirty/conflict 状态并视为已同步。
