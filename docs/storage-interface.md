...
- `readAgentRun(id)`
- `saveAgentRun(run)`
- `listActiveAgentRuns()` / `listNonterminalAgentRuns()`
...
- AgentRun 写入使用独立 `withAgentRunWriteLock()`，存放在 `agent-runs/<runId>.json` 与轻量 `agent-runs/active-index.json`；不保存原始 idempotency key、Prompt、工具完整结果或文本增量。
- AgentRun 在 local/S3 都是派生数据。S3 没有跨对象条件写，因此该锁、幂等与日配额仅保证单进程语义；多实例部署需先增加共享原子协调层。

## S3 Prefix 约束

- active prefix 只能通过 `getS3Prefix()` 和 `setS3Prefix(prefix)` 读取与更新。
- S3 初始化应在进程内复用同一个初始化结果或 in-flight Promise，避免每个请求重复 HEAD `notepads.json`、`thoughts.json` 和 `default.txt`。
- S3 notepad 正文 key 解析可以进程内缓存；切换 active prefix 时必须清空该缓存。
